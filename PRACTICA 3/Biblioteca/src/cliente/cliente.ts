...
//  cambiar `folio` por `id` en contrato/prestamo-response.dto.ts
//  y guardar.
//
...
form.addEventListener('submit', async (ev) => {
  ...
  if (res.status === 201) {
    const creado =
      (await res.json()) as PrestamoResponseDto;
    ...
    const error =
      (await res.json()) as ErrorResponseDto;
...
async function recargarLista(libroId: string): Promise<void> {
  const res = await fetch(
    `/api/prestamos?libroId=${encodeURIComponent(libroId)}`,
  );

  if (!res.ok) {
    lista.innerHTML =
      '<li class="vacio">No se pudo consultar</li>';
  ...
  if (prestamos.length === 0) {
    lista.innerHTML =
      '<li class="vacio">Sin prestamos para ese libro</li>';
  ...
  lista.innerHTML = prestamos
    .map(
      (p) => `<li>
        ...
        ejemplares ${p.ejemplares.join(', ')}
        &middot; socio ${p.socioId}
      </li>`,
    )
    .join('');
}
...
// ---------------------------------------------------------------------
//  Boton para consultar los prestamos del libro indicado
// ---------------------------------------------------------------------
